...
function ContinueCard({ item }: { item: WatchHistoryItem }) {
  ...
  const remainingSecs = item.duration !== null ? Math.max(0, item.duration - item.progress) : 0;
  ...
  return (
    <Link
      href={`/watch/${item.videoId}?t=${item.progress}`}
        ...
        <div className="space-y-1 p-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex shrink-0 items-center gap-1 text-[9px] text-white/20">
              ...
              <span>{timeAgo(Number(item.watchedAt))}</span>
            </div>
          </div>
        </div>
      </div>
